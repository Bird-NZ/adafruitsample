...
namespace Lesson_203
{
    ...
    public class Bmp280 : IBmp280
    {
        ...
        public async Task<float> ReadTemperatureAsync()
        {
            ...
            if (!_init) await BeginAsync();

            //Read the MSB, LSB and bits 7:4 (XLSB) of the temperature from the BMP280 registers
            var tmsb = ReadByte((byte) ERegisters.Bmp280RegisterTempdataMsb);
            var tlsb = ReadByte((byte) ERegisters.Bmp280RegisterTempdataLsb);
            var txlsb = ReadByte((byte) ERegisters.Bmp280RegisterTempdataXlsb); // bits 7:4

            //Combine the values into a 32-bit integer
            var t = (tmsb << 12) + (tlsb << 4) + (txlsb >> 4);

            //Convert the raw value to the temperature in degC
            var temp = BMP280_compensate_T_double(t);
        ...
        public async Task<float> ReadPreasureAsync()
        {
            ...
            var tmsb = ReadByte((byte) ERegisters.Bmp280RegisterPressuredataMsb);
            var tlsb = ReadByte((byte) ERegisters.Bmp280RegisterPressuredataLsb);
            var txlsb = ReadByte((byte) ERegisters.Bmp280RegisterPressuredataXlsb); // bits 7:4

            //Combine the values into a 32-bit integer
            var t = (tmsb << 12) + (tlsb << 4) + (txlsb >> 4);

            //Convert the raw value to the pressure in Pa
            var pres = BMP280_compensate_P_Int64(t);
        ...
        public async Task<float> ReadAltitudeAsync(float seaLevel)
        {
            ...
            var pressure = await ReadPreasureAsync();
